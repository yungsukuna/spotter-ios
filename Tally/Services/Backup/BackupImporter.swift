...
/// Imports a ``TallyBackup`` into a SwiftData store, merge-only: a record
...
enum BackupImporter {
    ...
    static func dryRun(_ backup: TallyBackup, into context: ModelContext) throws -> ImportReport {
    ...
    @discardableResult
    static func performImport(_ backup: TallyBackup, into context: ModelContext) throws -> ImportReport {
    ...
    private static func makePlan(_ backup: TallyBackup, context: ModelContext) throws -> Plan {
        guard backup.formatVersion <= BackupFormat.supportedVersion else {
            throw BackupError.unsupportedVersion(found: backup.formatVersion, supported: BackupFormat.supportedVersion)
        }
