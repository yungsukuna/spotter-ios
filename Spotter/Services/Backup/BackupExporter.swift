...
/// Builds a ``SpotterBackup`` from the current SwiftData store, and encodes it
...
enum BackupExporter {
    ...
    static func makeBackup(from context: ModelContext) throws -> SpotterBackup {
        var backup = SpotterBackup(
    ...
    static func encode(_ backup: SpotterBackup) throws -> Data {
        try BackupCoding.makeEncoder().encode(backup)
    }

    /// Write a fresh export to `Spotter-Backup-yyyy-MM-dd.json` inside
    ...
    static func writeExportFile(from context: ModelContext, calendar: Calendar = .current) throws -> URL {
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("Spotter-Backup-\(dayKey)")
